...
export const AuthProvider = ({ children }) => {
  ...
  const loginUserWithCredentials = async (email, password) => {
    try {
      ...
      } = await axios.post("https://drishyam-backend.shashankmishra4.repl.co/login", {
        email,
        password,
      });
      if (success) {
  ...
  const createUserWithCredentials = async (name, email, password) => {
    try {
      const {
        data: { user, token, success, message },
      } = await axios.post("https://drishyam-backend.shashankmishra4.repl.co/signup", {
        name,
        email,
        password,
      });
  ...
  const updateAccountDetails = async (id, name, email, password) => {
    const response = await axios.post(
      "https://drishyam-backend.shashankmishra4.repl.co/account",
      {
        id,
        name,
        email,
