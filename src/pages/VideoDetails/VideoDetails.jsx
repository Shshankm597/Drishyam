...
export default function VideoDetails() {
  ...
  const video = videos?.find((videoItem) => videoItem._id === videoId);
    ...
    if (newPlaylistInput && !getPlaylistByName(newPlaylistInput)) {
      try {
        const newPlaylistResponse = await axios.post(
          `https://drishyam-backend.shashankmishra4.repl.co/playlists`,
          {
            owner: user._id,
  ...
  const toggleInPlaylist = async (playlistId) => {
    ...
    try {
      const toggleInPlaylistResponse = await axios.post(
        `https://drishyam-backend.shashankmishra4.repl.co/playlists/${playlistId}`,
        {
          videoId: video._id,
        }
      );
